...
import { faMagnifyingGlass, faCalendarDays, faComment } from "@fortawesome/free-solid-svg-icons";
...
export default function BlogComponent() {
  ...
  const blogsContent = dict.blogTitles.map((title: string, index: number) => ({
    ...
    date: ["Sept. 23, 2020", "Oct. 10, 2020", "Nov. 5, 2020", "Dec. 1, 2020", "Jan. 20, 2021", "Nov. 5, 2020"][index],
  ...
  return (
    <div className="mx-6 custom-blog-margin xl:mx-[23%] mb-[9rem] py-16 transition-all duration-500" dir={lang === "ar" ? "rtl" : "ltr"}>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <aside className="p-6 rounded-2xl ">
          ...
          <div className="mb-3">
            ...
            <div className="flex flex-col gap-6">
              {blogsContent.slice(0, 3).map((blo, index) => (
                <div key={index} className="flex gap-4 items-start border-b border-gray-200 pb-5 last:border-0">
                  ...
                  <div className="flex flex-col justify-between">
                    <h3 className="text-[15px] font-semibold leading-snug mb-2 uppercase hover:text-pro cursor-pointer transition-all duration-300">
                      {blo.title.length > 40 ? blo.title.slice(0, 40) + "..." : blo.title}
                    </h3>
          ...
          <div className="my-6 mt-5">
            ...
            <p className="text-[#999999] text-bold text-[15px]">{dict.paragraphText}</p>
          </div>
        </aside>
      </div>
    </div>
